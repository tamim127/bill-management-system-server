...
const pdfkit = require("pdfkit");        // To generate PDF files
...
async function run() {
    try {
        ...
        app.put("/my-bills/:id", async (req, res) => {
            ...
            if (result.modifiedCount > 0) res.json({ success: true, message: "Bill updated successfully" });
        ...
        app.delete("/my-bills/:id", async (req, res) => {
            ...
            else res.json({ success: false, message: "Bill not found" });
        });

        // Download PDF of user's bills
